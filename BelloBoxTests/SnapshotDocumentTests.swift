...
final class SnapshotDocumentTests: XCTestCase {
    ...
    func testDocumentFromSnapshotUsesCapturedImageDimensionsForScaledDisplays() throws {
        let image = ScreenshotTestHelpers.stripedImage(width: 250, height: 200)
        let snapshot = DisplaySnapshot(
            displayID: 1,
            screenFrame: CGRect(x: 0, y: 0, width: 100, height: 80),
            scale: 1,
        ...
        XCTAssertEqual(document.baseImage.width, 75)
        XCTAssertEqual(document.baseImage.height, 38)
        XCTAssertEqual(document.scale, 2.5)
    }
}
